...
int main(int argc, char *argv[])
{
	...
	sol_broadcast = 1;
	/* TTL setting, protocol level - SOL_SOCKET, option - SO_BROADCAST */
	...
	/* broadcast port */
	addr_broadcast.sin_port = htons(atoi(argv[2]));

	while (!feof(fp)) {
		fgets(buff, BUFF_SIZE, fp);
		sendto(sock_broadcast, buff, strlen(buff), 0,
		       (struct sockaddr *)&addr_broadcast,
		       sizeof(addr_broadcast));
		sleep(1);
	}

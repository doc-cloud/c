...
int main(void)
{
	...
	flags |= O_NONBLOCK;
	/* O_NONBLOCK is set, it will wait if the socket is not available */
	...
	while (1) {
		...
		/* socket is not ready, read/write return -1, it is not an error */
		if ((n == -1) && (errno != EAGAIN)) {
		...
		inet_ntop(AF_INET, &cin.sin_addr, addr_p, addr_len);

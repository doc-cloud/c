...
/*****************  DES Function  *****************/
unsigned long OPENCOMM_DesExpandEncKey(unsigned char *pbDesKey,
				       unsigned long ulDesKeyLen,
				       unsigned char *pbDesEncKey,
				       unsigned long *ulDesEncKeyLen);

unsigned long OPENCOMM_DesExpandDecKey(unsigned char *pbDesKey,
				       unsigned long ulDesKeyLen,
				       unsigned char *pbDesDecKey,
				       unsigned long *ulDesDecKeyLen);

unsigned long OPENCOMM_DesEncRaw(unsigned char *pbDesEncKey,
				 unsigned long ulDesEncKeyLen,
...
unsigned long OPENCOMM_DesDecRaw(unsigned char *pbDesDecKey,
				 unsigned long ulDesDecKeyLen,
				 unsigned char *pbInData,
				 unsigned long ulInDataLen,
				 unsigned char *pbOutData,
...
int myic_DESDecrypt(unsigned char *pDesKey,
		    int nDesKeyLen,
		    unsigned char *pInData,
		    int nInDataLen, unsigned char *pOutData, int *pOutDataLen);

int myic_DESEncrypt(unsigned char *pDesKey,
		    int nDesKeyLen,
		    unsigned char *pInData,
		    int nInDataLen, unsigned char *pOutData, int *pOutDataLen);
...
void deskey(unsigned char *key, short edf, unsigned long *kn)
{
	...
	for (j = 0; j < 56; j++) {
		...
		m = l & 07;
		pc1m[j] =
		    (((unsigned long)key[l >> 3] & (unsigned long)bytebit[m]) ?
		     1 : 0);
	}
	for (i = 0; i < 16; i++) {
		if (edf == DE1)
		    ...
		    int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	...
	for (i = 0; i < loops; i++) {
		retval =
		    OPENCOMM_DesEncRaw(DesEncKeyBuf, EncKeyLen, pInData + i * 8,
				       8, pOutData + i * 8,
				       (unsigned long *)pOutDataLen);
		if (retval != 0)
			return retval;
	}
		    ...
		    int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	...
	for (i = 0; i < loops; i++) {
		retval =
		    OPENCOMM_DesDecRaw(DesDecKeyBuf, DecKeyLen, pInData + i * 8,
				       8, pOutData + i * 8,
				       (unsigned long *)pOutDataLen);
		if (retval != 0)
			return retval;
	   ...
	   int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	...
	rv = myic_DESEncrypt((unsigned char *)USER_PASSWORD_KEY,
			     strlen(USER_PASSWORD_KEY), padDate,
			     (int)padDateLen, pOutData, pOutDataLen);
	if (rv != 0) {
		if (padDate != NULL) {
			free(padDate);
		}
...
//数据加密
int DesEnc_raw(unsigned char *pInData,
	       int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	int rv;
	unsigned char *padDate = NULL;
	...
	rv = myic_DESEncrypt((unsigned char *)USER_PASSWORD_KEY,
			     strlen(USER_PASSWORD_KEY), pInData,
			     (int)nInDataLen, pOutData, pOutDataLen);
	if (rv != 0) {
		return rv;
	}
	return 0;
	   ...
	   int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	...
	//解密
	rv = myic_DESDecrypt((unsigned char *)USER_PASSWORD_KEY,
			     strlen(USER_PASSWORD_KEY), pInData, nInDataLen,
			     tmpPlain, pOutDataLen);
	if (rv != 0) {
		if (tmpPlain != NULL)
			free(tmpPlain);
		return rv;
...
//用户使用函数des解密
int DesDec_raw(unsigned char *pInData,
	       int nInDataLen, unsigned char *pOutData, int *pOutDataLen)
{
	int rv;
	//char                  padChar;

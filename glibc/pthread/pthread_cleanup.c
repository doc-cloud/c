...
{
	/* different args distinguish different function */
...
void *tfn1(void *arg)
{
	...
	/* load cleanup procedure, the number is different, in twice load */
	pthread_cleanup_push(cleanup, &a);
	/* load a thread exit cleanup function */
	pthread_cleanup_push(cleanup, &b);
	return NULL;
	/* should pair with pthread_cleanup_push */
	pthread_cleanup_pop(0);
	pthread_cleanup_pop(0);
	return NULL;
}

/* exit by pthread_exit */
void *tfn2(void *arg)
{
	int a = 1, b = 2;
